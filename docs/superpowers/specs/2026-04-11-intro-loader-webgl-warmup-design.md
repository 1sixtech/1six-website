...
- **인트로 타이밍**: 최소 0.7s, 하드캡 2.5s, 그 사이에서 WebGL ready 시점에 적응적으로 reveal
- **Phase 1 + Phase 2 동시 진행**: 인트로 + 프리워밍 + 컨텍스트 예산/동시성 정리
- **세션 내 2번째 홈 진입**: intro 완전 스킵 (`sessionStorage.introSeen` 플래그)
...
- **G3**: 인트로는 **적응형 타이밍** — 최소 0.7s, 최대 2.5s 하드캡. WebGL ready 시점에 따라 그 사이에서 결정된다. 빠른 재방문자에겐 0.7s에 reveal, 느린 첫방문자에겐 WebGL ready까지 대기.
- **G4**: 모바일/데스크톱 모두 동일 경험 (인트로는 반응형으로 작동).
- **G5**: `prefers-reduced-motion: reduce` 사용자는 인트로를 완전히 스킵한다.
- **G6**: 세션 내 2번째 이상 홈 진입(`/` → `/about` → `/` 링크 이동)은 인트로를 스킵한다 (`sessionStorage.introSeen`).
- **G7**: WebGL 컨텍스트 예산 초과(11 > 10) 이슈를 해소한다.
- **G8**: 기존 히어로 스크램블·thesis 핀/스와이프·theme toggle·accessibility 동작이 **동일하게 유지**된다.
...
| `src/lib/ascmosaic/texturedMesh.ts` | Modify | 중간 | `createVideoTexture(url, existingVideo?)` 시그니처 확장. existingVideo 제공 시 풀 비디오로 `THREE.VideoTexture` 래핑. 기존 경로는 폴백으로 유지. |
| `src/lib/ascmosaic/index.ts` | Modify | **높음** | `AscMosaic.addModel`과 `AscMosaic.dispose()` 내 video dispose 블록에 `videoPool.isPooled(video)` 체크 추가. pooled이면 `pause/src reset` 스킵. **라이브러리 핵심 코드**라 회귀 위험 주의. |
...
- **수정 파일**: 13개 (layout, page, globals.css, HeroSection, AsciiCanvas + 5 sub, ThesisSectionMobile, texturedMesh, ascmosaic/index)
...
| 메트릭 | Before | After (best case) | After (worst case) | 주석 |
|---|---|---|---|---|
| FCP (First Contentful Paint) | ~400ms | ~400ms | ~400ms | 인트로 자체가 즉시 렌더됨 (SSR) |
| LCP (Largest Contentful Paint) | ~1.2s | ~1.1s | ~2.9s | 히어로 캔버스가 reveal 후 노출. best=0.7s 인트로 + 0.4s reveal, worst=2.5s hardcap + 0.4s |
| 스크램블 완료 시점 | ~1.8s | ~2.5s | ~4.3s | intro reveal 트리거 + 기존 스크램블 1.8s 사이클 |
| `intro-lock` 해제 시점 | N/A | ~0.7s | ~2.5s | reveal onStart 시점 |
| Flicker on scroll | **있음** | **없음** | **없음** | 핵심 개선 목표 |
| Memory (GPU) | 8 컨텍스트 (모바일 평균) | 8 컨텍스트 (안정화) | 8 컨텍스트 | Phase 2로 상한 고정 |
| Memory (video elements) | 슬라이드별 재생성 | 11개 풀 (지속) | 동일 | 재할당 비용 제거 |
| 재방문자 (세션 내 2번째+) | 동일 | ~400ms | ~400ms | 인트로 스킵으로 즉시 진입 |

**Best case** (캐시 히트 + 빠른 디바이스): WebGL ready가 ~400-500ms에 완료 → 0.7s min 만료 → 즉시 reveal.
**Worst case** (첫 방문 + 느린 모바일 + 3G): WebGL ready가 ~2.5s에 도달 → hardcap에서 강제 reveal.

LCP 리그레션은 **브랜드 인트로라는 의도된 트레이드오프**이다. 빈 화면이 아닌 브랜드 애니메이션을 보므로 체감 품질은 향상된다. 또한 세션 내 재방문 시 스킵되므로 주요 사용 패턴에서 반복 부담이 없다.
...
### 5.2 Timeline (0.7s min, 2.5s hardcap, 적응형)

```
t = 0.00  IntroOverlay 마운트 (SSR html이므로 즉시 가시)
t = 0.00  videoPool.warmupAll() 시작 (병렬)
t = 0.00  홈페이지 AsciiCanvas × N (eager=true) 초기화 시작
t = 0.05  로고 fill 애니메이션 시작 (50ms 지연으로 hydration 안정화)
t = 0.65  로고 fill 완료 (duration 0.6s, power2.inOut)
t = 0.70  minDisplayTimer 완료
t = ???   webglReady

[분기 A: webglReady ≤ 0.70s  — 재방문 / 캐시 히트 / 빠른 디바이스]
t = 0.70  reveal GSAP onStart → 'intro:revealed' 발사 → intro-lock 제거 → sessionStorage.introSeen='1'
          → HeroSection useEffect 스크램블 스케줄 시작 (기존 300ms delay)
t = 1.00  스크램블 첫 글자 등장 (0.70 + 0.30)
t = 1.10  reveal GSAP 완료 (0.70 + 0.40)
t = 2.50  스크램블 완료 (0.70 + 기존 1.80 스크램블 사이클)

[분기 B: 0.70s < webglReady ≤ 2.50s  — 첫 방문 평균]
t = 0.65  로고 fill 완료, 이후 미묘한 pulse 대기
t = webglReady  reveal 시작
t = 이후 동일 (reveal 0.4s + 스크램블 1.5s)

[분기 C: webglReady > 2.50s  — 하드캡 강제 reveal]
t = 2.50  hardcap 만료 → reveal 시작 (WebGL 아직 준비 안 됐을 수 있음)
t = 2.90  reveal 완료
t = 2.90+ 각 섹션은 자체 초기화 완료 시점에 natural하게 등장 (기존 경로 폴백)
          → 첫 히어로는 아직 animate-pulse 상태일 가능성 → 스크램블과 함께 점진적 노출
```

**분기 C (worst case)** 의 의미: webglReady가 전혀 안 오는 극단적 경우 (일부 비디오 로드 실패, 극저사양 디바이스)에도 인트로가 무한히 걸리지 않도록 2.5s에서 강제 해제. 이때 일부 섹션은 여전히 기존 lazy init 경로로 떨어지지만 **사이드이펙트 없음** — 기존 동작 그대로.
...
**Why 0.6s?** 최소 인트로 시간이 0.7s로 타이트해졌으므로 fill은 그 이내에 완료되어야 한다. 0.05s delay + 0.6s duration = 0.65s 완료 → min 0.7s 내 여유 50ms.
...
import { getReadyKeys, HOMEPAGE_ASCII_TARGETS, computeExpectedReadyKeys } from '@/lib/introState';

const MIN_DISPLAY_MS = 700;   // 최소 노출 시간
const HARD_CAP_MS = 2500;      // 강제 reveal 시간

export function IntroOrchestrator({ fillRectRef, mainContentRef, overlayRef }) {
  const disposedRef = useRef(false);
  
  useEffect(() => {
    // prefers-reduced-motion OR 세션 내 재진입 → 인트로 전체 스킵
    // (inline script가 이미 intro-lock을 설정하지 않았음)
    if (!document.documentElement.classList.contains('intro-lock')) {
      runReveal({ skipAnimation: true });
      return;
    }
    
    // 1. videoPool 워밍업 시작 (병렬) — 11개 전부 (isNearby 무관)
    videoPool.warmupAll(HOMEPAGE_ASCII_TARGETS.map(t => t.textureUrl))
      .catch(err => console.warn('[intro] videoPool partial failure:', err));
    ...
    // 3. 기대되는 ready 키 목록 계산 (breakpoint + isNearby 고려)
    const expectedKeys = computeExpectedReadyKeys();
    const expectedCount = expectedKeys.length;
    
    // race-free: 모듈 Set에서 이미 도착한 ready 확인
    const isAllReady = () => {
      const ready = getReadyKeys();
      return expectedKeys.every(k => ready.has(k));
    };
    
    // 4. WebGL ready promise
    const webglReady = new Promise<void>(resolve => {
      if (isAllReady()) { resolve(); return; }
      const handler = () => {
        if (isAllReady()) {
          window.removeEventListener('ascii:ready', handler);
          resolve();
        }
      };
      window.addEventListener('ascii:ready', handler);
    });
    
    // 5. min timer + hardcap 경쟁
    const minTimer = new Promise<void>(r => setTimeout(r, MIN_DISPLAY_MS));
    const hardCap = new Promise<void>(r => setTimeout(r, HARD_CAP_MS));
    
    Promise.race([
      Promise.all([minTimer, webglReady]).then(() => 'ready'),
      hardCap.then(() => 'hardcap'),
    ]).then(reason => {
      if (disposedRef.current) return;
      if (reason === 'hardcap') {
        console.warn('[intro] hardcap reached, forcing reveal', { expectedCount, ready: getReadyKeys().size });
      }
  ...
  function runReveal({ skipAnimation }: { skipAnimation: boolean }) {
    const markSeen = () => {
...
/**
 * 인트로 단계에서 실제로 마운트되는 AsciiCanvas의 키 목록을 반환.
 * Phase 2의 isNearby 게이트를 고려해 초기 마운트 대상만 포함.
 * 오케스트레이터는 이 목록의 모든 키가 ready 이벤트를 발사한 후에만 reveal을 진행한다.
 */
export function computeExpectedReadyKeys(): string[] {
  const isMobile = typeof window !== 'undefined'
    && window.matchMedia('(max-width: 767px)').matches;
  const thesisKeys = isMobile
    ? ['thesis-1', 'thesis-2']  // Swiper 초기 active + nearby
    : ['thesis-1'];              // Desktop isNearby at index 0
  return ['hero', ...thesisKeys, 'graph', 'map', 'nevada-tv', 'nevada-trade'];
}
```

**주의**: `HOMEPAGE_ASCII_TARGETS.length = 11`이지만 Phase 2 isNearby로 인해 모바일 초기 마운트 시 thesis 4-6은 아직 마운트되지 않는다. 오케스트레이터는 **실제 마운트된 AsciiCanvas**의 준비를 기다려야 한다 — `computeExpectedReadyKeys()`가 이 목록을 동적으로 계산한다. 인트로는 **실제로 첫 화면에 노출될 컨텍스트들**만 기다리므로 불필요한 대기가 없다.

**videoPool은 11개 모두 warmup** — 초기 마운트 안 된 thesis 4-6도 나중에 사용자가 swipe해서 도달할 때 사용된다. 이는 네트워크/디코드 병렬 작업이라 AsciiCanvas 초기화와 독립적이다. videoPool 워밍업이 실패한 키는 해당 섹션이 진입할 때 기존 lazy 경로로 fallback하므로 안전.
...
**videoTexture.dispose() 주의**: 기존 `AscMosaic.addModel` (index.ts:202-223)와 `AscMosaic.dispose()` (index.ts:715-772)는 모델 정리 시 video element의 `src = ''`, `load()`를 호출하여 비디오를 죽인다. Pool에서 온 비디오는 이 로직을 타면 안 된다 (다른 섹션이 재사용해야 함).

**해결**: `videoPool.ts`에 pooled video를 추적하는 `WeakSet<HTMLVideoElement>` 추가. dispose 경로에서 pool 소유 여부를 체크하고 pooled video는 pause/src reset 스킵.
...
// warmup() 내부에서 pooledVideos.add(video)
```

```ts
// AscMosaic.addModel() / dispose() 의 dispose 블록 수정
if (video) {
  ...
  // pooled인 경우 그대로 재사용 가능하도록 둠
}
videoTexture.dispose();  // VideoTexture는 매번 새로 생성/dispose (가벼움)
```

**Why VideoTexture는 매번 dispose?** Three.js의 `VideoTexture`는 래퍼일 뿐이고, 실제 비싼 자원은 `HTMLVideoElement`이다. VideoTexture를 해제해도 비디오 element는 살아있으므로 다음 재사용 시 새 `VideoTexture(video)`를 만들면 된다. 이는 기존 dispose 경로와 역호환.
...
if (window.location.pathname === '/') {
  ...
  try {
    // G5: prefers-reduced-motion 사용자는 스킵
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      shouldIntro = false;
    }
    // G6: 세션 내 2번째 홈 진입은 스킵
    if (sessionStorage.getItem('introSeen') === '1') {
      shouldIntro = false;
    }
  } catch (e) {
    // sessionStorage 접근 실패 (private browsing 등)는 intro 보여줌 (fail-open)
  }
...
**중요**: `sessionStorage.setItem('introSeen', '1')`은 `IntroOrchestrator`의 `runReveal` 함수에서 설정한다 (§5.6 참고). 즉 **reveal이 실제로 발생한 경우에만** 플래그가 세팅됨 — intro 도중 사용자가 페이지를 떠나면 플래그 미세팅 → 다음 진입 시 다시 intro 재생. 의도된 동작.

**왜 IntroOverlay 마운트가 아닌 reveal 시점인가?** 인트로를 완주한 사용자만 "이미 봤다"고 간주. 중간 이탈자는 다음 진입 때 다시 보여줘야 브랜드 경험이 보장된다.
...
## 10. Open Questions & Resolutions (All Resolved)

1. **Q**: 워밍업 dispose 전략 — "셰이더 컴파일 + dispose" vs "video pool만"?  
   **A (Resolved by architecture)**: 자체 해결됨. 별도 `WebGLWarmup` 컴포넌트를 없애고 **홈페이지의 실제 AsciiCanvas 인스턴스에 `eager={true}`를 전파**하는 구조로 변경 (§4.1.1). dispose 없이 intro→runtime 끊김없이 연결. 사용자의 우선순위(기능 > UX > UI > 성능)에도 가장 부합 — race condition 여지 제거(기능), 캔버스가 이미 animating 상태로 사용자에게 도달(UX).

2. **Q**: 인트로 하드캡 2.5s는 충분한가?  
   **A (Resolved)**: 충분. 느린 모바일 기준 비디오 canplay 최악 1.5s + 셰이더 컴파일 ~500ms = 2s. 0.5s 여유. 유지.

3. **Q**: 첫 방문 vs 재방문 구분해서 intro 시간 차등 적용?  
   **A (Resolved)**: 적응형 타이밍으로 재설계. **MIN_DISPLAY_MS = 700ms**, hardcap 2500ms. WebGL ready 시점에 따라 0.7s ~ 2.5s 사이에서 자연스럽게 결정됨. 로고 fill 0.6s로 단축. 추가: 세션 내 재방문은 Q6으로 완전 스킵.

4. **Q**: videoPool 워밍업 일부 실패 시?  
   **A (Resolved)**: `Promise.allSettled`로 처리하되, orchestrator는 **allSettled 결과에 의존하지 않고** `ascii:ready` 이벤트를 직접 집계한다. 즉 실패한 비디오의 해당 AsciiCanvas는 기존 lazy init 경로로 폴백 → eventually onReady 발사 → orchestrator 카운터에 반영. 워밍업이 완전 실패해도 hardcap 2.5s에서 강제 reveal로 안전.

5. **Q**: 사용자가 intro 도중 페이지 이탈 (뒤로가기, 탭 전환) 시?  
   **A (Resolved after reconsideration)**: 
   - **탭 전환**: 브라우저가 background 탭의 rAF/타이머를 throttle → 인트로 progress 정지, videoPool warmup도 느려짐. 복귀 시 자동 재개. 문제 없음.
   - **뒤로가기**: 현재 `layout.tsx:147-149`의 `pageshow` 핸들러가 `e.persisted`일 때 `window.location.reload()` → bfcache 차단, 완전 새 로드. 새 로드에선 sessionStorage가 유지되므로 Q6 로직에 따라 스킵.
   - **링크 네비게이션 (`/about` → 다시 `/`)**: React Router가 IntroOrchestrator를 unmount → cleanup에서 GSAP timeline kill → 메모리 누수 없음. 다음 진입은 Q6 스킵.
   - **재검토 결과**: 특수 처리 불필요. 기존 답 유지.

6. **Q**: Next.js App Router의 client-side navigation에서 intro 재실행 방지?  
   **A (Resolved)**: **확정**. `sessionStorage.setItem('introSeen', '1')`은 `runReveal` 함수에서 reveal이 실제로 트리거된 경우에만 설정. 인트로를 완주하지 않은 사용자는 플래그 미세팅 → 다시 인트로 보여줌 (의도된 동작).

7. **Q**: 로고 fill 애니메이션 easing — `power2.inOut` vs `power3.out`?  
   **A (Resolved)**: `power2.inOut` 유지. 부드러운 양방향 easing이 단시간(0.6s) fill 애니메이션에 적합.
...
- `src/lib/videoPool.ts` (신규)
- `src/lib/introState.ts` (신규)
- `src/hooks/useIntroState.ts` (신규)
- `src/lib/ascmosaic/texturedMesh.ts` 수정 (`existingVideo` 파라미터 추가)
- `src/lib/ascmosaic/index.ts` 수정 (addModel/dispose 내 pooled 체크)
